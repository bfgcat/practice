...
# 	[-n <notify>]	= the time in hours that you gave notification for (if used, will send report to admin for action)
...
sub NotifyUser
{
	...
	print SENDMAIL "\nDear $user->{FullName},\n\tYour Perforce account \'$user->{User}\' on $options{port} has been inactive for at least $options{days} days and will be removed within 48  hours. If you still need this account active, please log into $options{port} at least once within the next 48 hours and your account will NOT be deleted.  Please open a ticket at \'(https://re-admin.i.turbinegames.com/REServiceTicket)\' if you need assistance.\n\nThis is an automated message from the RE group.";
	close(SENDMAIL);
}
...
sub NotifyClientUser
{
	...
	if ($client->{Owner} && $user{Email} && $user{FullName} !~ /build/i) {
		...
		print SENDMAIL $send_to;
		print SENDMAIL "Content-Type: text/plain\n\n";
		print SENDMAIL "Dear $user{FullName},\n\tYour Perforce workspace \'$client->{client}\' on $options{port} has been inactive for at least $options{days} days and will be removed within 48 hours. If you do not want this workspace to be deleted please log into $options{port}, switch to the workspace \'$client->{client}\' and sync at least one file.  By doing this, the workspace will remain intact. Please contact open a ticket at \'(https://re-admin.i.turbinegames.com/REServiceTicket)\' if you need assistance.\n\nThis is an automated message from the RE group.";
		close(SENDMAIL);
	}
}
